...
<div class="fixed inset-0 z-50 pointer-events-none [perspective:1200px] flex overflow-hidden">
  <motion.div
    initial={{ rotateY: 0 }}
    animate={isVisible ? { rotateY: -85, x: '-10%' } : { rotateY: 0, x: '0%' }}
    transition={{ duration: 0.75, ease: [0.16, 1, 0.3, 1] }}
    class="w-1/2 h-full bg-[#181818] origin-left border-r border-white/10 shadow-2xl"
  />
  <motion.div
    initial={{ rotateY: 0 }}
    animate={isVisible ? { rotateY: 85, x: '10%' } : { rotateY: 0, x: '0%' }}
    transition={{ duration: 0.75, ease: [0.16, 1, 0.3, 1] }}
    class="w-1/2 h-full bg-[#181818] origin-right border-l border-white/10 shadow-2xl"
  />
</div>`
  },
  {
...
<div class="fixed inset-0 z-50 pointer-events-none [perspective:1400px] flex overflow-hidden">
  <motion.div
    initial={{ rotateY: 90, opacity: 0 }}
    animate={isVisible ? { rotateY: 0, opacity: 1 } : { rotateY: 90, opacity: 0 }}
    transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
    class="w-1/2 h-full bg-[#1c1c1e] origin-right border-r border-white/15 shadow-2xl"
  />
  <motion.div
    initial={{ rotateY: -90, opacity: 0 }}
    animate={isVisible ? { rotateY: 0, opacity: 1 } : { rotateY: -90, opacity: 0 }}
    transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
    class="w-1/2 h-full bg-[#1c1c1e] origin-left border-l border-white/15 shadow-2xl"
  />
</div>`
  },
  {
...
>
  <path bind:this={paths[0]} fill="#262626" style="opacity: 0.6" />
  <path bind:this={paths[1]} fill="#1c1c1c" style="opacity: 0.85" />
  <path bind:this={paths[2]} fill="#121212" />
</svg>`
  },
  {
    id: 'radial-iris-mask',
...
<motion.div
  class="fixed inset-0 z-50 bg-[#121212] pointer-events-none origin-center"
...
<div class="fixed inset-0 z-50 pointer-events-none [perspective:1000px] flex items-center justify-center">
  <motion.div
    ...
    class="w-full h-full bg-[#181818] border border-white/10 shadow-2xl"
  />
</div>`
  },
  {
...
<motion.div class="fixed inset-0 z-50 flex pointer-events-none overflow-hidden">
  {#each Array(COLUMNS) as _, i (i)}
    <motion.div
      initial={{ y: '-100%' }}
      animate={isVisible ? { y: '0%' } : { y: '-100%' }}
      transition={{ duration: 0.65, ease: EASING, delay: i * 0.05 }}
      class="h-full flex-1 bg-[#181818]/95 backdrop-blur-md border-r border-white/5 last:border-r-0"
    />
  {/each}
</motion.div>`
  }
